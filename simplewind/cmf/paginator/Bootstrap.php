...
class Bootstrap extends Paginator
{
    ...
    protected function getLinks()
    {
        ...
        if($this->currentPage==1){
            $block['slider'] = $this->getUrlRange($this->currentPage, $this->currentPage + $side);
    ...
    {
        $html = '<li class="page-item"><div class="page-link" style="padding:5.5px"><select name="sldd" style="width:42px" onchange="location.href=this.options[this.selectedIndex].value;">';


        for ($x=1; $x<=$this->lastPage; $x++) {
            if($x == $this->currentPage){
                $html .= '<option selected value="?page='.$x.'">'.$x.'</li>';
            }else{
                $html .= '<option value="?page='.$x.'">'.$x.'</li>';
            }
